...
public:
    Solution(){}
    Solution(vector<string> wordlist)
    :_wordList(wordlist)
    {
        ...
        for( i=0;i<_wordMap.size();++i){//可优化,变成上三角或者下三角
            for(j=0;j<i;++j){
                if(canTransfer(_wordList[i],_wordList[j])){
                    _wordMap[i][j]=_wordMap[j][i]=true;
                }
                else{
                    _wordMap[i][j]=_wordMap[j][i]=false;
                }
            }
        }
    }

    void init(string &beginword,string &endword,vector<string> &wordlist){
          _wordList=wordlist;
        ...
        while(!_que.empty()){
            ...
            for(auto &i:nextWord){
                ...
                if(!_wordVisited.find(i)->second){
                    ...
                    cout<<" tmep= "<<temp.first<<"   temp.val= "<<temp.second<<endl;
